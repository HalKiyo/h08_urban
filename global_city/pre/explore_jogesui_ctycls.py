...
def explore_prf(citymask, rivnum, elevation, rivara):
    ...
    if max_key > 1:
        ...
        ara_indices = np.argwhere((rivnum == max_key) & (josui_array != max_key))
...
#---------------------------------------------------------------------------------------------------------------
    ...
    prf_save_path = f'{prf_save_dir}/city_{city_num:08d}.gl5'

    # save_flag
    if save_flag is True:
        josui_for_save.astype(np.float32).tofile(prf_save_path)
        print(f"{prf_save_path} saved")
    else:
...
#---------------------------------------------------------------------------------------------------------------
    ...
    # city sewage plant
    swg_save_path = f'{swg_save_dir}/city_{city_num:08d}.gl5'

    # save_flag
    if save_flag is True:
        gesui_for_save.astype(np.float32).tofile(swg_save_path)
        print(f"{swg_save_path} saved")
    else:
