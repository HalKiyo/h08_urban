...
def main():
    ...
    kumm_path   = f"{glob_dir}/dat/cty_lst_/gpw4/mcy_pop_water_12region.txt"
    ...
    save_path   = f"{cama_dir}/dat/cty_lst_/create_text/camacity_third.txt"

    wup = open_text(wup_path)
    full = open_text(full_path)
    latlon = open_text(latlon_path)
    kumm = open_text(kumm_path)
    ...
    for ind in range(1860):
        ...
        # 12 region
        line_kumm = kumm[ind]
        parts_kumm = line_kumm.split('|')
        parts_kumm = [item.strip() for item in parts_kumm]
        kummregion = parts_kumm[11]
        ...
        write_text(save_path, ind, ovlp_flg, dwn_flg, wup_pop, fll_pop, vld_pop, region, grid_num, country, lat, lon, city_name, kummregion)
        print(f'{ind} done')

def write_text(save_path, index, ovlp_flg, dwn_flg, wup_pop, fll_pop, vld_pop, region, grid_num, country, lat, lon, city_name, kummregion):
    if index == 0:
        with open(save_path, 'w') as file:
            file.write(f"{index+1}|{ovlp_flg}|{dwn_flg}|{wup_pop}|{fll_pop}|{vld_pop}|{region}|{grid_num}|{country}|{lat}|{lon}|{city_name}|{kummregion}\n")
    else:
